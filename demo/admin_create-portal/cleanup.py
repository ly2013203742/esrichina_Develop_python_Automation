...
try:
    ...
    try:
        for user in user_list:
            print('\nUser : ' + user.username + " # ")
            if user.fullName in ['Administrator', 'Esri', 'Esri Navigation',"arcgis"]:
                print('skipped')
                continue
    ...
    for user in user_list:
        if user.username == "arcgis" or user.username.startswith("esri_") or user.username.startswith("AVWORLD"):
            continue
        else:
